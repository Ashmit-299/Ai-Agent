...
class SentryManager:
    ...
    def setup(self):
        ...
        if not config.SENTRY_DSN or config.SENTRY_DSN.startswith('your-'):
            logger.warning("Sentry DSN not configured")
            return
            
        # Validate DSN format
        if not config.SENTRY_DSN.startswith('https://'):
            logger.warning("Invalid Sentry DSN format")
            return
...
class PostHogManager:
    ...
    def setup(self):
        ...
        if not POSTHOG_AVAILABLE:
            logger.warning("PostHog SDK not available")
            return
            
        if not config.POSTHOG_API_KEY or config.POSTHOG_API_KEY.startswith('your-'):
...
posthog_manager = PostHogManager()
