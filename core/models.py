...
class Script(SQLModel, table=True, extend_existing=True):
    ...
    title: str
    script_content: str
    script_type: str = Field(default="text")  # text, storyboard, etc.
    file_path: Optional[str] = None
    created_at: float = Field(default_factory=time.time)
    used_for_generation: bool = Field(default=False)

class Invitation(SQLModel, table=True, extend_existing=True):
...
class Analytics(SQLModel, table=True, extend_existing=True):
...
class SystemLog(SQLModel, table=True, extend_existing=True):
